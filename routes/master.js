...
router.get("/company-names", async (req, res) => {
    try {
        const result = await pool.query(
            `SELECT DISTINCT company_name FROM master ORDER BY company_name`
        );
...
router.get("/document-types", async (req, res) => {
    try {
        const result = await pool.query(
            `SELECT DISTINCT document_type FROM master ORDER BY document_type`
        );
...
router.get("/categories", async (req, res) => {
    try {
        const result = await pool.query(
            `SELECT DISTINCT category FROM master ORDER BY category`
        );
...
router.post("/", async (req, res) => {
    try {
        ...
        const result = await pool.query(
            `INSERT INTO master (company_name, document_type, category, renewal_filter)
             VALUES ($1, $2, $3, $4)
             RETURNING *`,
